...
public class LoggerSdk: OpenTelemetryApi.Logger {
    ...
    public func eventBuilder(name: String) -> OpenTelemetryApi.EventBuilder {
        ...
        return LogRecordBuilderSdk(sharedState: sharedState, instrumentationScope: instrumentationScope, includeSpanContext: true).setAttributes([
            "event.domain": AttributeValue.string(eventDomain),
            "event.name": AttributeValue.string(name)
        ])
    }

    public func logRecordBuilder() -> OpenTelemetryApi.LogRecordBuilder {
        LogRecordBuilderSdk(sharedState: sharedState, instrumentationScope: instrumentationScope, includeSpanContext: true)
    }
    ...
    func withoutTraceContext() -> LoggerSdk {
        LoggerSdk(sharedState: sharedState, instrumentationScope: instrumentationScope, eventDomain: eventDomain, withTraceContext: false)
    }
}
