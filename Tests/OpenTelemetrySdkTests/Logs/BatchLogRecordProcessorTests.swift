...
class BatchLogRecordProcessorTests: XCTestCase {
    ...
    func testMaxLimit() {
        ...
        let processor = BatchLogRecordProcessor(logRecordExporter: waitingExporter, scheduleDelay: 10_000, maxQueueSize: maxQueueSize, maxExportBatchSize: maxQueueSize / 2)
    ...
    func testForceExport() {
        ...
        let processor = BatchLogRecordProcessor(logRecordExporter: waitingExporter, scheduleDelay: 10_000, maxQueueSize: 10_000, maxExportBatchSize: 2_000)
        let loggerProvider = LoggerProviderBuilder().with(processors: [processor]).build()
        let logger = loggerProvider.get(instrumentationScopeName: "BatchLogRecordProcessorTest")
        for _ in 0 ..< 100 {
            logger.logRecordBuilder().emit()
...
class BlockingLogRecordExporter: LogRecordExporter {
    ...
    func export(logRecords _: [ReadableLogRecord]) -> ExportResult {
        cond.lock()
